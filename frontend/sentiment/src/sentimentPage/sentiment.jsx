...
const SentimentsPage = () => {
    ...
    const topFiveReviews = (reviews) => {
        const filteredReviews = reviews.filter(review => review.reviewText.length < 2000);
                ...
                {isLoading ? (
                    ...
                    <>
                        <div className="block space-y-4 md:flex md:space-y-0 md:space-x-4 md:rtl:space-x-reverse">
                            <button onClick={openModal} className=" reviewmargin block w-full md:w-auto text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" type="button">
                                Reviews
                            </button>
                        </div>
                        {showModal && (
                            <div id="top-left-modal" data-modal-placement="top-left" className="fixed top-0 left-0 right-0 bottom-0 z-50 w-full bg-black bg-opacity-50 flex justify-center items-center">
                                <div className="bg-white rounded-lg border border-gray-500 shadow-md p-8 max-w-5xl w-full max-h-screen overflow-y-auto">
                                    <div className="flex items-center justify-between pb-4 border-b">
                                        ...
                                        <button onClick={closeModal} class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="top-left-modal">
                                    ...
                                    <div className="mt-4 flex justify-between">
                                        <div className="w-1/2 mr-2 p-4 bg-green-100 border border-green-300 rounded-lg shadow sm:p-6 md:p-8 dark:bg-green-100 dark:border-green-300">
                                            <h4 className="text-4xl font-semibold mb-2">Positive Reviews <hr class="h-px my-6 bg-gray-500 border-0 dark:bg-gray-500"/></h4>
                                            <ul>
                                                {topPositiveReviews.map((review, index) => (
                                                    <li key={index} className="mb-2" style={{textAlign: 'center'}}>{review.reviewText} <hr class="h-px my-6 bg-green-300 border-0 dark:bg-green-300"/></li>
                                                ))}
                                            </ul>
                                        </div>
                                        <div className="w-1/2 ml-2 bg-red-100 border border-red-300 rounded-lg shadow sm:p-6 md:p-8 dark:bg-red-100 dark:border-red-300">
                                            <h4 className="text-4xl font-semibold mb-2">Negative Reviews <hr class="h-px my-6 bg-gray-500 border-0 dark:bg-gray-500"/></h4>
                                            <ul>
                                                {topNegativeReviews.map((review, index) => (
                                                    <li key={index} className="mb-2" style={{textAlign: 'center'}}>{review.reviewText} <hr class="h-px my-6 bg-red-300 border-0 dark:bg-red-300"/></li>
                                                ))}
                                            </ul>
                                        </div>
                                    </div>
